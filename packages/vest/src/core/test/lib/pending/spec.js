...
describe('module: pending', () => {
  ...
  describe('export: removePending', () => {
    describe('When testObject it not pending or lagging', () => {
      ...
      it('Should keep state unchanged', () => {
        ...
        expect(suite.getState(suiteId)).toEqual(currentState);
      });
    });
    ...
    describe('When testObject is either pending or lagging', () => {
      ...
      describe('When in lagging', () => {
        ...
        it('Should remove test from lagging', () => {
          expect(suite.getState(suiteId).lagging).toContain(testObject);
          removePending(testObject);
          expect(suite.getState(suiteId).lagging).not.toContain(testObject);
          expect(suite.getState(suiteId)).toMatchSnapshot();
        });
      });
    });
  ...
  describe('export: setPending', () => {
    ...
    it('Should set supplied test object as pending', () => {
      expect(suite.getState(suiteId).pending).not.toContain(testObjects[0]);
      setPending(testObjects[0]);
      expect(suite.getState(suiteId).pending).toContain(testObjects[0]);
    });
    ...
    describe('When a field of the same profile is in lagging array', () => {
      ...
      it('Should remove test from lagging array', () => {
        expect(suite.getState(suiteId).lagging).toContain(testObjects[0]);
        setPending(testObjects[0]);
        expect(suite.getState(suiteId).lagging).not.toContain(testObjects[0]);
        expect(suite.getState(suiteId)).toMatchSnapshot();
      });
